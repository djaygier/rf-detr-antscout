...
def train_one_epoch(
    ...
    for data_iter_step, (samples, targets) in enumerate(
        ...
        mixup_alpha = getattr(args, 'mixup', 0.0)
        cutmix_alpha = getattr(args, 'cutmix', 0.0)
        if mixup_alpha > 0.0 or cutmix_alpha > 0.0:
            # Decide whether to apply augmentation at all
            # We use a combined probability of 0.5 for the batch if either is enabled
            # or we can use the alpha as a probability indicator (standard is often 1.0)
